...
/* 获取今天的第n天的日期 */
-(NSDate *)getForeDays:(NSInteger)fore {

    NSDateComponents *components = [self.calendar components: self.unit fromDate:[NSDate date]];
    if (fore != -1) {
        components.day += (fore + 1);
    }else{
        components.day += fore;
    }
...
- (NSString *)holidayInDate:(NSDate *)date {
    ...
    NSString *holidayKey = [NSString stringWithFormat:@"%zd-%zd",month,day];
    ...
    NSString *chinesHolidayKey = [NSString stringWithFormat:@"%zd/%zd",chineseMonth,chineseDay];
...
+ (NSDate *)toDateWithString:(NSString *)str {
    NSDateFormatter *formate = [[NSDateFormatter alloc] init];
    [formate setDateFormat:@"yyyy-MM-dd"];
...
+ (BOOL)isYesterdayWithDate:(NSDate *)date
{
    ...
    [formate setDateFormat:@"YYYY-MM-DD"];
    NSString *todayStr = [formate stringFromDate:[NSDate date]];
    NSDate *nowDate = [formate dateFromString:todayStr];
...
+ (NSInteger)daysWithStart:(NSDate *)start end:(NSDate *)end
{
    NSDateFormatter *formate = [[NSDateFormatter alloc] init];
    [formate setDateFormat:@"YYYY-MM-DD"];
    NSString *startStr = [formate stringFromDate:start];
    NSDate *startDate = [formate dateFromString:startStr];
    
    NSString *endStr = [formate stringFromDate:end];
    NSDate *endDate = [formate dateFromString:endStr];
    
    NSCalendar *calendar = [[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian];
    NSDateComponents *cmps = [calendar components:NSCalendarUnitDay fromDate:endDate toDate:startDate options:0];
